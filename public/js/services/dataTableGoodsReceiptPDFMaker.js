'use strict';
angular.module('app').service('dataTableGoodsReceiptPDFMaker', [
  '$filter',
    ...
    return {
      make : function (pageSize, pageOrientation, title, fileName, tableHeader, data, type, columnNames) {
        ...
        function getTotalQuantity(products){
          var total = 0;
            ...
            _(products).each(function(product){
              ...
              total = math.add(total, product.quantity);
            });
          return total;
        }

        function getQuantityReceipt(products){
          var total = 0;
          _(products).each(function(product){
            total = math.add(total, product.quantityReceipt);
          });
          return total;
        }
        ...
        data = _(data).map(function (rowData) {
          var finalObject = {};
          _(objectColumnNames).mapObject(function(key) {
            var valueProperty = '';
            if (key === 'customer' || key === 'supplier') {
              valueProperty =  getPersonName(rowData);
            } else if (key === 'products.totalQuantity') {
              valueProperty = getTotalQuantity(rowData.products);
            } else if (key === 'products.quantityReceipt') {
              valueProperty = getQuantityReceipt(rowData.products);
            } else {
              valueProperty = Object.byString(rowData, key);
            }
            finalObject[key] = valueProperty.toString();
          });
          return finalObject;
        });
